...
ReceiptForm::ReceiptForm():JForm(0, 0, 0, 0), rft_(RFT_UNKNOWN)
{
}
...
ReceiptForm::ReceiptForm(SHORT x, SHORT y, SHORT w, SHORT h, const std::vector<std::string>& items, const std::string& title /* = std::string */)
: JForm(x, y, w, h), rft_(RFT_UNKNOWN), title_(title)
{
	int yy = y + 4;
	int xx = x + 5;
	...
	{
		JLable* label;
		label = new JLable(xx, yy, 13, 1, *it, this);
		yy += 2;
		items_[*it] = label;
	}
...
void ReceiptForm::OnKeyEvent(JEvent* e)
{
	...
	if(key == KEY_ENTER && e->GetSender() == btnReturn_)
	{
		...
		case RFT_DEPOSIT:
			...
			break;
		case RFT_WITHDRAW:
			dynamic_cast<MainMenuForm*>(form)->GetItem()[2]->SetCurrent();
			break;
		case RFT_TRANSFER:
			dynamic_cast<MainMenuForm*>(form)->GetItem()[3]->SetCurrent();
			break;
...
{
	std::map<std::string, JLable*>::const_iterator it;
