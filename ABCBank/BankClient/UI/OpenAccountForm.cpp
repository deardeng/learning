...
OpenAccountForm::OpenAccountForm(SHORT x, SHORT y, SHORT w, SHORT h,
	...
	: JForm(x, y, w, h),
	  title_(title)
{
	lblName_ = new JLable(11, 5, 13, 1, "CUSTOMER NAME:", this);
	editName_ = new JEdit(26, 5, 21, 1, "", this);
	lblNameTip_ = new JLable(50, 5, 21, 1, "长度3-10位，字母或数字", this);

	lblId_ = new JLable(13, 8, 12, 1, "IDENTIFY ID:", this);
	editId_ = new JEdit(26, 8, 21, 1, "", this);
	lblIdTip_ = new JLable(50, 8, 12, 1, "长度18位", this);

	lblPass_ = new JLable(8, 11, 17, 1, "ACCOUNT PASSWORD:", this);
	editPass_ = new JEdit(26, 11, 21, 1, "", this, JEdit::EM_PASSWORD);
	lblPassTip_ = new JLable(50, 11, 11, 1, "长度6-8位", this);

	lblPass2_ = new JLable(9, 14, 16, 1, "REPEAT PASSWORD:", this);
	editPass2_ = new JEdit(26, 14, 21, 1, "", this, JEdit::EM_PASSWORD);
	lblPass2Tip_ = new JLable(50, 14, 16, 1, "同上", this);

	lblMoney_ = new JLable(19, 17, 16, 1, "MONEY:", this);
	editMoney_ = new JEdit(26, 17, 21, 1, "", this);
	lblMoneyTip_ = new JLable(50, 17, 19, 1, "小数最多两位", this);
...
void OpenAccountForm::OnKeyEvent(JEvent* e)
{
	...
	if (key == KEY_ESC)
	{
		...
		dynamic_cast<MainMenuForm*>(form)->GetItem()[0]->SetCurrent();
		form->ClearWindow();
		form->Show();
		e->Done();
	}
