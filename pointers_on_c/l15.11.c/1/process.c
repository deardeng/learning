...
int process_request( void )
{
	...
	else if( sscanf( request, "new %20[^,],%hu,%lf %1s",description, &quantity, &price_each, left_over ) == 3 ){
		new_part( description, quantity, price_each );
	}	
			
	else if( sscanf( request, "%10s %lu,%hu,%lf %1s", request_type,&part_number, &quantity, &price_each, left_over ) == 5 &&( strcmp( request_type, "buy" ) == 0|| strcmp( request_type, "sell" ) == 0 ) ){
		buy_sell( request_type[ 0 ], part_number, quantity, price_each );
	}
	else if( sscanf( request, "%10s %lu %1s", request_type, &part_number,left_over ) == 3 &&( strcmp( request_type, "delete" ) == 0|| strcmp( request_type, "print" ) == 0 ) ){
		if( request_type[ 0 ] == 'd')
			delete( part_number );
		else
			print( part_number );
	...
	else if( sscanf( request, "print %s %s", request_type, left_over ) == 2&& strcmp( request_type, "all" ) == 0 ){
		print_all();
	}
	else {
